var reqs = ['Lasor', 'Ship', 'LoDash'];
define( reqs, function( Lasor, Ship, _ ) { 
  ...
  function drawLasors(canvas, style) {
    ...
    if( !_.isNull( this.lasors() ) )
      for(var i = 0, lasors = this.lasors(); i < lasors.length ; i++) { lasors[i].draw(canvas, style); }
    canvas.restore();
  }
  ...
  function shoot(ship) {
    ...
    var angle = this.angleBetweenObjects(this, ship);
    var velocityY = 1;
  ...
  function Enemy(config) {
    ...
    console.log('thisd' , startingOriginX);
