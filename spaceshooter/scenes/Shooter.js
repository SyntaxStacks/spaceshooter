var reqs = ['GameCanvas', 'GameInput', 'UI', 'Enemy', 'Hero', 'LoDash'];
define( reqs, function( GameCanvas, GameInput, UI, Enemy, Hero, _ ) { 
  ...
  function Shooter(config) {
    ...
    function updateSprites(input) {
      ...
      var enemies = getEnemies();
      _.map( enemies, function( enemy ) { enemy.update(getShip()); });
    }

    function removeDestroyedObjects(){
      var enemies = getEnemies();

      enemies = _.map( enemies, function( enemy ) {
        if( _.isUndefined( enemy ) || enemy.isDestroyed() ) { return null; }
        return enemy;
      });
